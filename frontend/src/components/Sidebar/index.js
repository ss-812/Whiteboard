...
const Sidebar = () => {
  ...
  const fetchCanvases = async () => {
    try {
      const response = await axios.get('https://whiteboard-be-x3f8.onrender.com/api/canvas/list', {
  ...
  const handleDeleteCanvas = async (id) => {
    try {
      await axios.delete(`https://whiteboard-be-x3f8.onrender.com/api/canvas/delete/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      fetchCanvases();
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      const response = await axios.put(
        `https://whiteboard-be-x3f8.onrender.com/api/canvas/share/${canvasId}`,
        { email },
        {
          headers: { Authorization: `Bearer ${token}` },
        }
